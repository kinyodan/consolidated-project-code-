...
class ManageSearchEngineDataHelper
{
    ...
    */
    ...
    {
        $this->search_engine_settings = config('craydle.search_engine.courses.settings');
        $this->index_name = config('craydle.search_engine.courses.index_name', 'craydel_courses');
        $this->app_id = config('craydle.search_engine.app_id');
        $this->app_key = config('craydle.search_engine.api_key');
        $this->data = $data;
    ...
    */
    public function push(): CraydelInternalResponseHelper
    {
        try{
            ...
            $result = $index->saveObjects($this->data);
            if(!$result->valid()){
