...
class OnOpportunityStudentEnrolledCommandController
{
    use CanSendEmail;

    /**
     * Process the opportunity after the user moves the opportunity to student enrolled
     *
    ...
    protected static function review(?object $opportunity){
        ...
        $is_missing_course_requirements = call_user_func(function () use($opportunity){
            ...
            if(!empty($opportunity_contact_id) && !empty($course_name) && !empty($institution_name)){
                ...
                if(!empty($contact_email)){
                    ...
                    (new self())->sendEmail(
                        ...
                        $contact_email,
                        sprintf(
                            LanguageTranslationHelper::translate('course_application.workflow.notifications.course_application_student_enrolled_subject'),
                            !empty($contact_name) ? " ".$contact_name : "",
                            $institution_name
                        ),
                        view('emails.leads.workflow.enrolment-done',[
