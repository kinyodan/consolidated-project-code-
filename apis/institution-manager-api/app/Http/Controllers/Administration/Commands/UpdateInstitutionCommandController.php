...
class UpdateInstitutionCommandController
{
    /**
     ...
     * @return JsonResponse
     */
    public function update(string $institution_code, Request $request): JsonResponse
    {
        try{
            $validate = $this->validate($institution_code, $request);
            ...
            $payload = [
                ...
                'temp_logo_path' => !empty($this->institutionController->stagedFilePath) ? $this->institutionController->stagedFilePath : null,
                'has_updates' => 1,
            ...
            DB::transaction(function () use($payload, $institution_code, &$hasUpdated){
                $hasUpdated = DB::table((new Institution())->getTable())
            ...
            if($hasUpdated){
                ...
                return $this->institutionController->respondInJSON((new CraydelJSONResponseType(
                    true,
                    LanguageTranslationHelper::translate('institutions.success.updated')
                )));
            }else{
                return $this->institutionController->respondInJSON((new CraydelJSONResponseType(
                    false,
     ...
     */
    protected function validate(string $institution_code, Request $request): CraydelInternalResponseHelper
    {
        $institution_code = trim($institution_code);
        ...
        $check_duplicate = DB::table((new Institution())->getTable())
            ->where('country_code', trim($country_code))
