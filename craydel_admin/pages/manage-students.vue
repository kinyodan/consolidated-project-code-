  ...
  <v-app>
    <div class="container-fluid">
      ...
      <div class="row column1">
        <div data-app>
          <SchoolFormEditing v-if="editing" :editing="editing" :itemData="itemData"></SchoolFormEditing>
          <SchoolForm v-else :editing="false"></SchoolForm>
          <v-dialog
          ...
          >
            <template>
              <v-container>
                <template>
                  <v-card >
                    <ShowClassDetails :classDetails="schoolDetails" ></ShowClassDetails>
                    <v-card-actions>
                      <v-spacer></v-spacer>
...
export default {
  name: 'ManageSchools',
  layout:'Default',
  components:{ValidationObserver,ValidationProvider,SchoolFormEditing,SchoolForm},
  head(){},
  created() {
    this.listSchools();
  ...
  methods: {
    ...
    async deleteItemConfirm(item){
      let response =await ClassesService.deleteSchool(item.id)
      if(response.data.status){
