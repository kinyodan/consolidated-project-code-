...
describe('Testing Employers page ', () => {
  ...
  it("LOGIN: to preserve token and navigate to Dashboard pathways and then to employer_list page", ()=>{
    ...
    //
    // click access assesment ..
    cy.contains('Add new Employer').click();
    cy.wait(6000);
    ...
    // ####################  START opening Add new employer view in sequence and autofill form     
    //
    //
    // Get form inputs and autifill values 

     cy.get('input[name="Name"]') // 4.
            .type('Cypress-test-item-employer') // 5
    
     cy.get('.ck-editor__editable') // 4.
            .type('DESCRIPTION: Cypress test item employer') // 5
     ...
     cy.get('.select_e2').select('ACADEMIA')
                ...
                cy.get('.b-table tr > td').first().each( (item , index) =>{
                    ...
                    .wrap(item)
                      ...
                      cy.get('.etd-name').first()
                          .should('have.attr', 'href', "/pathways/employers/employer/?id=1");
                      ...
                      cy.get('[data-id="Cypress-test-item-employer"]').click()
                      cy.wait(1000);
                      cy.get('[data-delete="Cypress-test-item-employer"]').click({force: true})
