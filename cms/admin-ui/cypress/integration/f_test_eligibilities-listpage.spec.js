...
describe('Testing Eligibilities page ', () => {
  ...
  it("LOGIN: to preserve token and navigate to Dashboard pathways and then to employer_list page", ()=>{
    ...
    //
    //
    // click access Employers ..
    cy.contains('Eligibilities').click();
    cy.wait(6000);
    // end click Employers
    //
    //
    ...
    //
    // click access assesment ..
    cy.contains(' Add new Eligibilites').click();
    cy.wait(6000);
    // end click Add new Employer
    //
    //
    ...
    // ####################  START opening Add new eligibilities view in sequence and autofill form     
    //
    //
    // Get form inputs and autifill values 

     cy.get('input[name="Name"]') // 4.
            .type('Cypress-test-item-eligibility') // 5
    
     cy.get('.select_e1').select('Kenya')

     cy.get('.select_e2').select('Nursing')

     cy.get('.select_e3').select('Academic Eligibility')

     cy.get('.select_e4').select('Cypress test item specialization')

     cy.get('.select_e5').select('Medicine')
    ...
    //
    //
    // ###################  END opening Add new employer view in sequence and autofill form     
                ...
                cy.get('.b-table tr > td').first().each( (item , index) =>{
                    ...
                    .wrap(item)
                      ...
                      cy.get('.etd-name').first()
                          .should('have.attr', 'href', "/pathways/eligibilities/new-eligibility?id=1");
                      ...
                      cy.get('[data-id="Cypress-test-item-eligibility"]').click()
                      cy.wait(1000);
                      cy.get('[data-delete="Cypress-test-item-eligibility"]').click({force: true})
