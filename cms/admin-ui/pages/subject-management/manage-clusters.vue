    ...
    methods: {
        ...
        async createItem(item) {
           ...
           //start loading
            ...
            let response = await ClusterManagementService.createCluster(this.formData);
            if (response.data.status) {
        ...
        async updateItem(item) {
          let self = this; 
          let response = await ClusterManagementService.updateCluster(this.current_item,item.cluster_name,item.country_id);
          if (response.data.status) {
              self.FormSubmittedSuccess = response.data.status;               
              self.FormSubmittedSuccessText = response.data.message;  
              this.editing=false
        ...
        async getData(page) {
          ...
          var data_set = [];
          var data_final =[];
          let response = await ClusterManagementService.listClusters(page);
          if (response.data.status) {
            data_load = response.data.data.items;
            ...
            var ad =[]
            data_load.forEach((cluster,index) => {
               if(index==0){

               }else{
                 ad =  merge(data_load[0].clusters, cluster.clusters);
               }
            ...
            if (self.$route.query.id){
              self.editing = true;
              var query_item =self.table_data[0].findIndex(x => x.clusters_id == self.$route.query.id);
              self.current_item=self.table_data[0][query_item].clusters_id;
              self.editPopup(self.current_item);
            }
        ...
        editPopup(item){
          ...
          this.formData.cluster_name = item.cluster_name;
        ...
        async delete_this(item) {
          let self = this; 
          let response = await ClusterManagementService.deleteClusters(item.clusters_id);
          if (response.data.status) {
              self.FormSubmittedSuccess = response.data.status;               
              self.FormSubmittedSuccessText = response.data.message;  
              this.getData();   
                ...
                <div class="card-body">
                          <div class="">
                              ...
                              <a  href="javascript: void(0);" role="button" aria-expanded="false" @click="newPopup()" >
                                  <span class="bck-to-0">&nbsp;&nbsp;new Clusters&nbsp;&nbsp;</span>
                              </a>
                          </div>
                          ...
                          <BlockUI  v-if="new_subject">
                              ...
                              <span class="close" @click="close()"></span>
                                <h4> {{ form_header_text }} Cluster </h4>
                                <div class="row">
                                  <div class="col-12">
                                      <form v-on:submit.prevent="formSubmit" class="form-horizontal" role="form">                                     
                                        <label  class="pd-hld move-left" >Cluster Name</label>
                                        <input v-model="formData.cluster_name" type="text"
                                         class="form-control"
                                         placeholder="Name" name="Cluster name"/>
                    ...
                    <div class="table-responsive mb-0 " >
                        ...
                        @filtered="onFiltered">
                              <template  v-slot:cell(cluster_name)="data">
                                  <div class="details_field px-2">  
                                      <div style='margin-left:10px;'>
                                        {{ data.item.cluster_name }}</br></br>
