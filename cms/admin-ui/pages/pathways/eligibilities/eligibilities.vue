    ...
    methods: {
        async getEligibilities() {
           ...
           //start loading
            ...
            let getligibilitieslist = EligibilitiesService.getlist().then(response => {

              if (response.status) {
                self.eligibility_items = response.data.data.items;
        ...
        handleFilters() {
          ...
          //get the data
          this.getpageEligibilities()
        },
        handleClearFilters() {
          this.filterData.country_code = null
          this.getpageEligibilities()
        },
        handlePageChange_elgbty(value) {
          //get the data
          this.getpageEligibilities(value)
        },
        delete_this_eligibility(id){

            let self = this;
            this.loadingData_del = true;
            //this.getData(this.currentPage)

            EligibilitiesService.deleteligibilty(id).then(response => {
              let data = response.data
              if (data.status) {
                this.getpageEligibilities(self.currentPage)
                this.loadingData_del = true;
                self.$router.go('/pathways/eligibilities/eligibilities');
        ...
        getpageEligibilities(page) {
          ...
          let self = this
          let getEligibilitiespage = EligibilitiesService.getpage(page, elgtypFilterData).then(response => {

            let eligibility_response_data = response.data
            if (eligibility_response_data.status) {
              self.tableData = eligibility_response_data.data.items;
              self.totalRows = eligibility_response_data.data.items_count;
              var perpage_convert =parseInt(eligibility_response_data.data.items_per_page);
              self.perPage = perpage_convert;
              self.currentPage = eligibility_response_data.data.currentPage;
              self.loadingData = false
        ...
        activate_item(url , id){
          let activate_response = GlobalService.activate(this.activate_api_url,id)

        },
        deactivate_item(url,id){
          let activate_response = GlobalService.deactivate(this.activate_api_url,id)
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                      ...
                      to="/pathways/eligibilities/new-eligibility">
                    ...
                    <div class="table-responsive mb-0">
                      <b-table
                        ...
                        <template v-slot:cell(code)="data">
                          <a href="#" class="text-body font-weight-bold">{{ data.item.code }}</a>
                        </template>
                        ...
                        <template v-slot:cell(status)="data">
                              ...
                              <nuxt-link
                                :to="{path:'/pathways/eligibilities/new-eligibility', query:{id:data.item.id} }"
                          ...
                          <ul class="list-inline mb-0"  id='del-section-anchor'>
                            ...
                            <li class="list-inline-item actions-edt-del-1" >
                              ...
                              <span class="`confirm-bx-${data.item.id}` actions-edt-del-in0"  v-if="confirm_check==data.item.id">
                                ...
                                <span class="px-2 etd-confirm text-primary" :data-delete="`${data.item.name}`" v-b-tooltip.hover title="Confirm delete" @click="delete_this_eligibility(data.item.id)">  
                      ...
                      <div class="col">
                        <div class="dataTables_paginate paging_simple_numbers float-right">
                          <ul class="pagination pagination-rounded">
                            <!-- pagination -->
                            <b-pagination @input="handlePageChange_elgbty" v-model="currentPage" :total-rows="rows"
