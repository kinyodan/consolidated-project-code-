<template>
  <div class="modal fade in" id="show_assessments_details" tabindex="-1">
    <div class="modal-dialog create_new_event">
      <div class="modal-content full-width">
        <div class="modal-content-inner">
          <div class="modal_header">
            <h2 class="modal_title">Assessments Done</h2>
              ...
              >
                ...
                <template v-slot:expanded-item="{ headers, item, isMobile }">
                  <td :colspan="headers.length" class="school-grid-mobile" v-if="isMobile">
                    ...
                    <div class="school-grid-mobile__row">
                      <div class="v-data-table__mobile-row__header">Career Paths</div>
                      <div class="v-data-table__mobile-row__cell">{{ item.career_paths }}</div>
                    </div>
                    <div class="school-grid-mobile__row">
                      <a :href="`{{ item.report }}`">View Report</a>
                    </div>
                  </td>
                </template>
                <template v-slot:item.report="{ item }">
                  <a :href="`{{ item.report }}`" title="View Report">
                    <v-icon>
                      mdi-chart-bar
                    </v-icon>
                  </a>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        {text: 'Career Paths', value: 'career_paths', sortable: false},
        {text: 'Report', value: 'report', align: 'center', sortable: false},
      ],
  ...
  methods: {
      ...
      this.students = [
        {
          ...
          student_stream: 'A',
          career_paths: 'Media & Entertainment, Designing & Art, Management & Marketing',
          report: 'https://workspace.craydel.com/ideal-report?profile=9511143768'
        },
        {
          id: 2,
    ...
    closeModal() {
      $('#show_assessments_details').modal('hide');
    },
  },
}
</script>
